...
interface SmsExclusionKeywordDao {
    ...
    /** 사용자/채팅 추가 키워드 엔티티 조회 */
    @Query("SELECT * FROM sms_exclusion_keywords WHERE source != 'default' ORDER BY source ASC, keyword ASC")
    ...
    /** 키워드 일괄 추가 (중복 무시) */
    @Insert(onConflict = OnConflictStrategy.IGNORE)
    suspend fun insertAll(entities: List<SmsExclusionKeywordEntity>)
