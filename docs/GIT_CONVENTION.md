...
> 커밋, 브랜치, 푸시, PR에 관한 모든 규칙은 이 문서에서만 관리한다.
> 커밋/푸시/PR 규칙은 `CLAUDE.md`에 중복 정의하지 않고, 반드시 이 문서를 참조한다.
...
- `master`,`develop` 에 직접 force push 금지
...
- 요청 문구에 따라 동작을 구분한다.
- **"PR 작성해줘" 요청 시**:
  - 실제로 `gh` 명령어로 PR을 생성하지 않는다.
  - 텍스트(PR 제목/본문)만 작성하여 제공한다.
- **"PR 생성해줘" 요청 시**:
  - `gh pr create`로 PR을 실제 생성한다.
  - 생성 전에 브랜치 푸시/베이스 브랜치/제목·본문을 확인하고 진행한다.
  - 생성 결과(URL, 번호)를 반드시 사용자에게 공유한다.
