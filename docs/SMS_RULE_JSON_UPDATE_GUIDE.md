...
룰 개수와 sender별 커버리지는 문서에 스냅샷으로 고정하지 않는다.
운영 현황은 항상 아래 원본에서 확인한다.

- 룰 원본: `app/src/main/assets/sms_rules_v1.json`
- 회귀 테스트: `SmsRegexRuleAssetTest`
- 운영 fail 커버: `scripts/sms_origin_rule_audit.py`
