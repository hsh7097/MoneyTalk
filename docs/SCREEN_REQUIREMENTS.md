...
| 항목 | 스펙 |
|------|------|
| 게이팅 | AI 크레딧 부족 시 메시지 시도하면 광고 다이얼로그 |
| 대기 메시지 | 임시 저장 → 광고 시청 완료 후 재전송 |
| 크레딧 표시 | 서브타이틀: "AI 크레딧: N" |
| 프리로드 | rewardAdEnabled 변경 시 자동 프리로드 |
| 실패 처리 | 광고 로드/표시 실패 → 크레딧 미지급, 대기 메시지 해제 |
...
| 항목 | 스펙 |
|------|------|
| 동작 | ExpenseRepository.deleteDuplicates() + IncomeRepository.deleteDuplicates() |
| 결과 | 거래처 규칙을 먼저 재적용한 뒤 지출은 동일 금액/거래처/카드/일시 항목 중 고정지출·통계 제외·메모가 보존된 항목을 우선 남기고, 수입은 동일 원문/발신/일시/금액 항목 중 smsId가 있는 항목을 우선 남긴 뒤 삭제 건수 스낵바 표시 |
...
| AI 크레딧 | 일반 질문 1회 전송 시 1크레딧 차감, 광고 시청 완료 시 RTDB `reward_ad_chat_count`만큼 충전 |
| 크레딧 확인 | 설정 → AI 크레딧에서 잔액, 광고 충전, 최근 원장 내역 표시 |
