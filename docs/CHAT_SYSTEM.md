...
#### 날짜 해석 규칙
- "이번달" → 사용자 설정 월 시작일을 반영한 앱 기준 이번 달 시작일 ~ 오늘
- "지난달" → 사용자 설정 월 시작일을 반영한 앱 기준 지난달 시작일 ~ 종료일
- "3개월간" → 최근 3개월
- 연도 미지정 → 올해로 가정
...
- 카테고리 예시는 실제 displayName을 사용한다. 예: 카페는 `카페/간식`, 배달앱은 `배달`.
- `배달`은 `식비`의 하위 leaf 카테고리다. `카테고리별 지출`처럼 전체 분해는 `식비`와 `배달`을 분리하고, 사용자가 `식비`를 특정해서 묻는 조회는 하위 포함 총합을 우선 제공한다.
- AI 답변 버블은 `**강조**` 문법을 bold 텍스트로 렌더링해 원문 Markdown 토큰을 노출하지 않는다.
...
| "배달 포함된건 배달로 바꿔" | `update_category_by_keyword` |
