...
| embedding | List<Float> -> JSON | 임베딩 벡터 (768차원, outputDimensionality 지정) |
...
| 배달 | 배달의민족, 배민, 요기요, 쿠팡이츠, 땡겨요 등 | 식비 하위 leaf 카테고리. 카테고리별 화면에서는 식비와 분리, 명시적 식비 상담 조회에서는 하위 포함 |
| 카페/간식 | 스타벅스, 이디야 등 | 카페, 베이커리, 아이스크림/빙수 |
...
| 사용자 요청 | 실행되는 액션 |
|-----------|------------|
| "쿠팡 결제는 온라인쇼핑으로 분류해줘" | `update_category_by_store` (정확히 일치) |
| "배달의민족 포함된건 배달로 바꿔줘" | `update_category_by_keyword` (부분 일치) |
| "ID 123번 지출을 교통으로 변경해줘" | `update_category` (개별 변경) |

### 동일 가맹점 일괄 업데이트

사용자가 카테고리를 변경하면 동일 가맹점(storeName) 지출 전체에 카테고리가 일괄 적용됩니다.
- 홈/내역 화면에서 카테고리 수정 시 → 해당 가맹점의 모든 지출 카테고리 동시 변경
- 채팅에서 카테고리 변경 성공 시 → `CategoryReferenceProvider` 캐시 자동 무효화
- 동일 가맹점 비교는 저장된 거래처명을 변경하지 않고, 비교 시 내부 공백과 대소문자를 제거한 키로 수행합니다. 예: `가나 다라`와 `가나다라`는 같은 거래처로 처리됩니다.
