...
│   └── CategoryDetailPageContent    ← 월별 콘텐츠 (요약 + 차트 + 거래 목록)
│       ├── MonthNavigationHeader    ← 월 네비게이션 (좌우 화살표)
│       ├── CategoryDetailHeroCard   ← 이번 달 카테고리 지출 + 예산 상태 요약
│       ├── SpendingTrendSection     ← 누적 지출 추이 (카테고리 전용)
│       ├── CategoryTransactionListHeader ← 최근 내역 제목 + 정렬 토글
...
| CategoryDetailPageContent | 월별 콘텐츠 (요약 + 차트 + 거래 목록) | [CategoryDetailScreenKt](../app/src/main/java/com/sanha/moneytalk/feature/categorydetail/ui/CategoryDetailScreen.kt) |
...
| CategoryDetailHeroCard | 카테고리 월 지출과 예산 상태 요약 카드 | [CategoryDetailScreenKt](../app/src/main/java/com/sanha/moneytalk/feature/categorydetail/ui/CategoryDetailScreen.kt) |
| CategoryTransactionListHeader | 거래 목록 제목과 최신순/가격순 정렬 토글 | [CategoryDetailScreenKt](../app/src/main/java/com/sanha/moneytalk/feature/categorydetail/ui/CategoryDetailScreen.kt) |
...
```
TransactionEditActivity               ← 거래 편집/추가 (별도 Activity)
└── TransactionEditScreen             ← 거래 상세+편집 컨테이너 (다이얼로그/피커/온보딩 관리)
    ├── TransactionEditDetailContent  ← 카드형 상세 UI (라이트/다크, Hero + 정보 카드)
    ...
    ├── CategoryAddDialog             ← 카테고리 추가 다이얼로그 (설정 화면과 공유)
...
| 함수 | 설명 | 참조 |
|------|------|------|
| TransactionEditScreen | 거래 상세/편집 화면 컨테이너 | [TransactionEditScreenKt](../app/src/main/java/com/sanha/moneytalk/feature/transactionedit/ui/TransactionEditScreen.kt) |
...
└── CategorySettingsScreen             ← 카테고리 설정 메인 화면
    ...
    │   └── "+ 카테고리 추가" 버튼
    ├── CategoryAddDialog              ← 커스텀 카테고리 추가 공통 다이얼로그
