...
```
TransactionEditActivity               ← 거래 편집/추가 (별도 Activity, 지출+수입)
└── TransactionEditScreen             ← 거래 편집 폼 (컴팩트 행 기반)
    ├── CompactReadOnlyRow            ← 읽기 전용 행 (날짜/시간/카테고리 등 picker용)
    ├── CompactEditRow                ← 편집 가능 행 (금액/가게명/메모 등, 포커스 시 X버튼)
...
| 함수 | 설명 | 참조 |
|------|------|------|
| TransactionEditScreen | 거래 편집/추가 전체 화면 (지출+수입) | [TransactionEditScreenKt](../app/src/main/java/com/sanha/moneytalk/feature/transactionedit/ui/TransactionEditScreen.kt) |
