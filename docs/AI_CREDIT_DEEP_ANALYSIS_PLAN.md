...
|----------|------|-------:|
| 간단 질문 | "이번 달 식비 얼마야?" | 1 |
| 기본 기간 분석 | "이번 달 지출 요약해줘" | 2 |
| 월별 추세 분석 | "올해 어디서 지출이 늘었어?" | 3 |
| 가족 기준 재무 상담 | "4인 가족 기준 적절해?" | 5 |
| 장기 심층 리포트 | "6개월 흐름과 절약 계획 세워줘" | 8 |
...
- 기존 리워드 채팅 횟수 모델을 크레딧 모델로 확장
- 분석 유형별 필요 크레딧 산정
- 크레딧 예약 차감/성공 확정/실패 환불 흐름 추가
- 리워드 광고 보상을 크레딧 충전으로 전환
- UI 문구를 "무료 상담 횟수"에서 "AI 크레딧"으로 변경

### Phase 3. 크레딧 확인 페이지

- 설정 또는 채팅 상단에서 진입
- 잔액, 충전, 사용 내역, 크레딧 기준 표시
- Composable map 및 화면 요구사항 문서 갱신
...
- 보상형 광고 실패 시 현재 정책대로 사용자에게 불리하지 않게 처리되는가
- 구매 토큰이 중복 지급되지 않는가
