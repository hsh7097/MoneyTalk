...
> 현재 크레딧 사용 단가는 [AI_CREDIT_USAGE_PLAN.md](AI_CREDIT_USAGE_PLAN.md)를 우선한다.
> 이 문서의 0/1/3/10 차등 크레딧 표는 심층 분석 고도화 검토용 과거 초안이다.
...
| 분석 유형 | 예시 | 크레딧 |
|----------|------|-------:|
| 단순 조회 | "이번 달 식비 얼마야?" | 과거 초안: 0 |
| 가벼운 조언 | "카페 지출 줄일 방법 추천해줘" | 과거 초안: 1 |
| 기본 기간 분석 | "이번 달 지출 요약해줘" | 과거 초안: 3 |
| 월별 추세 분석 | "올해 어디서 지출이 늘었어?" | 과거 초안: 3 |
| 장기 심층 리포트 | "6개월 흐름과 절약 계획 세워줘" | 과거 초안: 10 |

실제 Gemini 토큰 사용량은 내부 로깅/비용 추적용으로만 사용한다.
2026-07-08 기준 운영 정책은 채팅 1회 1크레딧이며, `LocalChatQueryRouter`가 처리하는 단순 조회는 사용자 크레딧은 차감하되 Gemini query analyzer/final answer/Rolling Summary 호출은 생략한다.
...
- 기존 리워드 채팅 횟수 모델을 크레딧 모델로 확장 (1차 완료: 채팅 1회 1크레딧, 광고 충전 원장화)
- 분석 유형별 필요 크레딧 산정 (보류: 현재 운영 정책은 채팅 1회 1크레딧)
