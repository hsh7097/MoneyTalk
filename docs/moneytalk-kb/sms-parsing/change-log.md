...
## 2026-09-14 - 수신시각 보정 출처의 삭제 추적

- 입력 필터에서 스킵되거나 writer에서 정본으로 보정되는 수신시각 차이 SMS도, 기존 원문·숫자 잔액·60초 중복 근거가 확인된 지점에서 출처 ID를 연결한다. DAO에 도달하지 않아 삭제 후 다른 ID로 다시 저장될 수 있던 경계를 보완했다.
- DB/현재 배치 중복 필터와 지출/일반 수입 보정에 대해 삭제 서비스 → 추적 재로드 → 양쪽 출처 재수집을 확인하는 Room 회귀 4개를 추가했다. API 36 에뮬레이터에서 수집 41개와 출처 삭제 8개가 통과했으며, 임의 동일 행 ID 연결 금지 및 잔액 없는 반복 거래 보존도 함께 확인했다.
