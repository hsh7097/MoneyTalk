...
이 문서는 2026-09-14 배포 준비 중 확인한 소스와 검증 기록이다. 새 버전의 배포 완료 기록이 아니다. 월 경계 Pager 오류를 수정하고, `8c94ab5` 기준 빌드·자동 검사와 표준 Release의 에뮬레이터 사용 흐름을 검증했다. 사용자가 `이어서 해줘`로 `1.0.5 (23)` 버전 변경·보호 브랜치 통합·원격 푸시·Play 배포의 계속 진행을 승인했다. 버전 커밋 `4a69b6b` 기준 23번 AAB/APK의 빌드와 산출물 검증을 완료했다. 파일 선택 도구가 오류를 반환했으나 실제 Play Console에서 내부 초안 `4`의 23번 번들 등록과 노트 저장을 확인했다. 아직 출시를 게시하지 않았으며 보호 브랜치 통합과 원격 푸시도 수행하지 않았다.
...
| 23번 AAB 해시 / Play 결과 | 아래 최종 산출물 절에 SHA-256 기록 / 내부 초안 등록 확인 | 초안 `4`에 `23 (1.0.5)`, API 36 번들 1개와 노트 저장. 기존 16번 제외, 미게시 |
...
- 내부 테스트의 기존 빈 초안 `4`에 출시 이름과 안내 문구를 입력하고 파일을 선택했다. `setFiles`가 Chrome의 `Not allowed` 오류를 반환했지만, 별도 권한 변경 없이 초안 저장 후 실제 Console에서 `23 (1.0.5)`, API 36 번들 1개가 등록된 것을 확인했다. 기존 16번은 초안에서 제외됐으며 노트 저장도 완료했다. 도구 오류만으로 업로드 미완료를 단정하지 않는다. 출시·심사 제출은 아직 수행하지 않았고 기존 22를 재사용하는 변경도 하지 않았다.
- Console의 관리형 게시는 사용 중지 상태여서 심사 승인이 나면 자동 공개된다. 초안 입력 전 실사에서는 미전송·검토 대기 변경이 없었다. 업로드 인증서와 Play 앱 서명을 구분해 확인했으며 키는 변경하지 않았다. 초안 세부 정보 확인 후 제출·심사·공개 상태를 다시 확인한다.
...
2. 실제 Console에 등록된 23번의 세부 정보와 초안 구성을 확인하고 승인된 트랙에 제출한다. 테스트 트랙의 구형 번들 및 정책 경고 상태를 확인하며, 관리형 게시가 꺼져 있어 승인이 나면 자동 공개됨을 반영해 진행한다.
