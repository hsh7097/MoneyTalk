...
description: 7월 Play 배포본과 9월 로컬 변경을 구분하고 1.0.5 후보의 기능, 검증 결과, 미완료 배포 조건을 기록한다.
...
이 문서는 2026-09-14 배포 준비 중 확인한 소스와 검증 기록이다. 새 버전의 배포 완료 기록이 아니다. 월 경계 Pager 오류를 수정하고, `8c94ab5` 기준 빌드·자동 검사와 표준 Release의 에뮬레이터 사용 흐름을 검증했다. 사용자가 `이어서 해줘`로 `1.0.5 (23)` 버전 변경·보호 브랜치 통합·원격 푸시·Play 배포의 계속 진행을 승인했다. 버전 커밋 `4a69b6b` 기준 23번 AAB/APK의 빌드와 산출물 검증을 완료했다. Play 내부 초안에는 이름·안내 문구만 입력했으며 Chrome 파일 접근 권한 문제로 파일 업로드·출시가 진행되지 않았다. 보호 브랜치 통합과 원격 푸시도 아직 수행하지 않았다.
...
| 23번 AAB 해시 / Play 결과 | 아래 최종 산출물 절에 SHA-256 기록 / 미업로드 | 내부 초안 이름·노트 입력만 수행. 파일 업로드와 출시 미완료 |
...
| 배포용 23번 산출물 | 패키지·버전·서명·운영 설정·16KB 정렬 검사 통과 | 아래 해시로 식별한 `MoneyTalk-1.0.5-release23.aab` 및 APK. Play 파일 업로드는 미수행 |
...
- 실제 프로덕션 22는 API 36이며, 정책 문제에 포함된 활성 번들은 Alpha의 `15 (1.0.0)`과 내부 테스트의 `16`으로 둘 다 API 35다. 현재 코드의 SDK를 다시 올려야 하는 문제로 단정하지 않는다. [Google Play 대상 API 요구사항](https://support.google.com/googleplay/android-developer/answer/11926878?hl=en)에 따라 새 후보 제출 후 각 활성 트랙과 정책 상태를 확인한다.
- 내부 테스트의 기존 빈 초안 `4`에 출시 이름과 안내 문구를 입력했다. 파일 선택 창까지 연결했으나 `setFiles`가 Chrome의 `Not allowed` 오류로 실패했다. 사용자에게 확장의 파일 URL 접근 권한 활성화를 요청한 상태다. 파일 업로드·출시·심사 제출은 수행하지 않았으며, 기존 22를 테스트 트랙에 재사용하는 변경도 하지 않았다.
- Console의 관리형 게시는 사용 중지 상태여서 심사 승인이 나면 자동 공개된다. 초안 입력 전 실사에서는 미전송·검토 대기 변경이 없었다. 업로드 인증서와 Play 앱 서명을 구분해 확인했으며 키는 변경하지 않았다. 업로드 재개 후 제출·심사·공개 상태는 다시 확인한다.

## 브랜치와 보존 기록

배포 준비 브랜치는 `codex/release-1.0.5`다. 현재 HEAD에 포함된 작업 브랜치 4개를 정리한 뒤 로컬에는 이 브랜치와 `develop`, `master`가 남았다. 보호 브랜치 통합과 원격 푸시는 사용자 승인 범위에 포함되지만 아직 수행하지 않았다. Git Credential Manager를 통한 push dry-run은 성공했으며 실제 원격 반영을 뜻하지 않는다. 완료 커밋과 원격 반영 결과는 진행 후 추가한다.
...
1. 승인된 보호 브랜치 통합·푸시를 수행하고 완료 커밋과 원격 반영 결과를 기록한다. 서명 설정·난독화 규칙·라이브러리를 임의 변경하지 않는다.
2. Chrome 확장의 파일 접근 문제가 해소되면 위 해시로 검증한 23번 AAB를 업로드하고 승인된 트랙에 제출한다. 테스트 트랙의 구형 번들 및 정책 경고 상태를 확인하며, 관리형 게시가 꺼져 있어 승인이 나면 자동 공개됨을 반영해 진행한다.
3. 실제 Play 설치 경로에서 AI 응답을 확인한다. 심사·게시·정책 해소 상태는 Console의 확인 결과로 갱신한다.

스토어 안내 초안은 로컬 `artifacts/release-1.0.5-20260914/release-notes-ko.txt`에 작성했다. 해당 파일이 있다는 사실은 Play 제출·게시 완료를 뜻하지 않는다.
