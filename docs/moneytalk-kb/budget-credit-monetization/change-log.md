...
| 날짜 | 근거 | 변경 | 영향 문서 | 메모 |
|---|---|---|---|---|
| 2026-09-08 | 예산 금액 경계 교차 리뷰 | 범위 초과 시 저장·모드 전환 차단, 비율 표시와 저장 계산 통일 | `01-feature-flow.md`, `../settings/package-reference/03-rendering-action.md` | 200% 계산 오버플로로 기존 카테고리 예산이 삭제되는 경로를 막고, 작은 전체 예산에서 큰 비율로 변환하는 사례를 회귀 테스트에 추가. |
| 2026-09-07 | `BudgetBottomSheet`, `BudgetInputAmounts`, `BudgetInputAmountsTest` | 현재 전체 예산에 따른 비율 저장 계약 보강 | `01-feature-flow.md`, `../settings/package-reference/03-rendering-action.md` | 비율 입력 뒤 전체 예산만 바꿀 때 이전 금액을 저장하던 경로를 보정. |
