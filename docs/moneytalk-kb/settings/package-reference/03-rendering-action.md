...
- 설정 목록 외부 여백은 20dp로 통일한다. 공통 설정 섹션/행은 복원한 테마 표면과 일반 아이콘 `primary`, 부제목 `onSurface` 60%, 화살표 `onSurface` 30%를 사용한다. 위험 행은 `error`를 유지하고, 카테고리 정리/알림 토글 custom 행도 기존 60dp 최소 높이와 22dp 아이콘·14dp 간격을 사용한다.
