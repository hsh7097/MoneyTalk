...
type: domain
title: Settings 도메인
description: MoneyTalk 설정 탭의 예산, AI 서비스 상태, 테마, 백업/복원, 카드 보유, Google Drive, 코치마크 흐름을 설명한다.
...
> 상태: verified
> 기준: 2026-07-12 release 1.0.3 UI 순회와 API key 설정 제거 이후 구현 확인

Settings는 네 번째 하단 탭이며 앱 환경, 예산, AI 서비스/크레딧, 데이터 백업/복원, Google Drive, 카드 보유 여부, SMS/거래처/카테고리 설정 진입을 담당한다. 사용자가 Gemini API key를 입력하거나 저장하는 메뉴는 없다.
...
- 테마, 월 시작일, 예산, 카드 보유와 AI 서비스 상태는 `package-reference/02-data-viewmodel.md`를 본다.
