...
Chrome의 인증된 Firebase Console에서 `sms_origin`만 내려받아 분석했다. 운영 RTDB의 룰·설정은 변경하지 않았다.

- snapshot: `moneytalk-258e5-default-rtdb-sms_origin-export (1).json`
- SHA-256: `EA0011F28CE7192475DF5AD47D98F211B60C4B81198BDFAA5A585E9C6C834691`
- 241개 row, `count` 합 476회. 과거 outcome은 실패 223개, 성공 18개이므로 전체 사용자 문자 정확도를 대표하지 않는다.
- 동일 표본의 지출 regex Fast Path는 변경 전 **3개/3회**, 변경 후 **219개/448회**다. 비거래 2개/5회, 원문 검증 불가 3개/6회, 추가 검토 17개/17회, `policy_false_positive=0`이다.
- 기존 192개 표본도 재검증해 지출 165개, 수입 11개, SKIP 10개, 외화 보류 6개로 기존 결과를 유지했다.
- 이 수치는 로컬 룰 재생 결과다. Vector/Gemini 최종 결과, 운영 전체 정확도, 수신부터 저장까지 실제 지연 시간, 실제 비용 절감률을 측정한 것은 아니다.

| 반영 형식 | 변경 |
|---|---|
| 카카오뱅크 카드결제 | 잔액의 양수·음수 모두 허용. 기존 양수 전용 key는 INACTIVE로 남겨 기기 DB에서도 교체되게 함 |
| 카카오뱅크 출금 | 거래 금액·사용처·거래 시각을 잔액과 분리 |
| KB국민 승인·체크 | 카드번호가 붙은 승인 헤더와 체크카드 사용 멀티라인 보강 |
| 신한 체크·지원금 포함 승인 | 사용 금액을 잔액 및 지원금 표기와 구분 |
| 현대 지마켓스마일 승인 | 카드 상품명과 멀티라인 사용처 허용 |
| 신한은행 출금 | 날짜와 시각이 나뉜 본문 처리 |
| 삼성 일반·가족카드 | 일반 승인과 가족카드, 일시불·할부 처리 |
| 우리카드 이용안내 | 안내 헤더 뒤 실제 승인 내역 처리 |

구조가 불명확한 지원금 사용 안내, 후정산, 원문 마스킹 표본은 자동 지출 룰로 억지 승격하지 않았다. 테스트에는 운영 고객의 이름·계좌·거래처를 복사하지 않고 합성 문자를 사용한다.

추출 JSON은 필수 결제 여부·양의 정수 금액·사용처를 검증하고 선택 필드의 null을 허용한다. 배치 번호 중복/범위 초과/소수 금액을 정상 거래로 보정하지 않는다. RTDB 룰 필드가 잘못된 경우 해당 노드만 제외해 다른 정상 룰을 살린다.

## 2026-07 입력 데이터와 검증 범위
