...
| [02-filter-surfaces.md](02-filter-surfaces.md) | 화면 필터, 카드 노출 필터, 통계 제외, SMS 입력 제외, export 필터를 계층별로 설명한다. | 필터 변경의 영향 범위를 구분할 때 |
...
- History bottom sheet의 임시 화면 필터는 `HistoryFilter.kt`와 `HistoryViewModel.kt`를 본다.
- 카드 숨김이 화면/집계/노티까지 이어지는지는 `02-filter-surfaces.md`의 카드 노출 필터를 본다.
- SMS 제외 키워드/발신자 차단은 신규 파싱 입력과 기존 화면 노출이 다르게 동작할 수 있으므로 `02-filter-surfaces.md`를 먼저 본다.
