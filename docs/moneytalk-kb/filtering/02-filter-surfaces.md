...
달력에서 날짜 상세로 이동할 때는 `TransactionDetailFilter`로 현재 유형/카테고리/카드/고정/정렬을 전달한다. 상세 재조회에도 같은 선택 조건, SMS 제외 키워드, 숨김 카드 정책을 적용한다. 카드가 선택되면 수입을 숨기며 통계 제외 거래는 목록에 남긴다. 기존 날짜 extra만 있는 진입은 수입 우선·숨김 카드만 적용하는 동작을 유지한다.
