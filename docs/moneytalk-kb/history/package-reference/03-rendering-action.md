...
- `FilterTabRow`는 보기 전환/검색/추가와 필터를 두 행으로 구분한다. 필터가 적용되어도 `FilterActionButton`을 계속 표시하며, `FilterStatusChip`의 본문은 상태 설명이고 오른쪽 48dp X만 초기화한다.
...
- 확인 항목: 필터 적용 → 같은 필터 다시 열기 → 취소 시 선택 유지 → 전용 X 초기화, 목록/달력 전환 후 날짜 상세 진입, 큰 글자에서 월 금액/날짜/필터 버튼 접근, 밝은/어두운 화면의 지출 중립색·수입 강조색.
