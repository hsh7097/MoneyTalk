...
- 홈·카테고리 상세와 함께 실효 월을 Compose state로 관찰하고 페이지 수를 계산하도록 변경했다. 관찰 불가능한 시계 변화가 캐시와 측정에 서로 다른 페이지 수를 주는 충돌 경로를 제거한다.
- `MonthPagerStateTest`는 실제 Pager에 공용 helper를 연결해 월 증가 후 복귀, 시계 후퇴, 시작일 설정으로 페이지 수가 줄거나 늘어나는 경계를 검증한다.

## 2026-09-14 - 다음 날 복귀 시 달력 날짜 갱신

- 월/시작일만 사용하던 달력 캐시 키에 오늘 날짜를 포함하고 `ON_RESUME`에서 날짜를 갱신한다. 당일 거래가 있어도 전날의 미래일 판정으로 금액과 날짜 상세가 숨겨지는 기존 문제를 수정했다.
- `CalendarDateRolloverTest`는 같은 월과 같은 거래 입력에서 14일→15일→16일 복귀를 재현하여 지출·수입 표시, 날짜 상세 클릭, 무지출일 합계를 확인한다. API 36 에뮬레이터에서 통과했으며 기존 큰 글자 금액 레이아웃 2개도 통과했다.

## 2026-09-08 - 단건 정리와 수입 검색

- 롱클릭의 화면별 ViewModel → 공용 단건 수정/삭제 서비스 연결, 기존 상세 편집과의 책임을 rendering/data 문서에 반영했다.
- 지출/수입 전체 기간 검색, 수입 필터·합계, 취소된 검색의 페이지 덮어쓰기 방지 경계를 기록했다.
- 수입/이체의 기존 합계 정의 차이는 제품 검토의 별도 후속으로 연결했다. 최종 빌드·저장/화면 테스트 결과는 통합 로그 확인 후 기록한다.
