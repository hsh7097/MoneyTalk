...
| [02-data-contract.md](02-data-contract.md) | embedding API 출력, StoreEmbedding 저장 model, 유사도 threshold contract를 설명한다. | 모델/차원/schema/threshold 변경 |
| [03-extension-points.md](03-extension-points.md) | 임베딩 확장 지점과 변경 금지에 가까운 경계를 설명한다. | 정책/전파/배치 처리량 변경 |
| [04-files-checklist.md](04-files-checklist.md) | 수정 전후 확인 파일과 검증 질문이다. | 리뷰 전 누락 점검 |
| [05-file-inventory.md](05-file-inventory.md) | embedding 전체 파일 역할 인덱스다. | 파일 후보가 애매할 때 |
...
- 모델/차원/threshold를 건드리면 `02-data-contract.md`와 [../project-context/02-threshold-registry.md](../project-context/02-threshold-registry.md)를 같이 확인한다.
